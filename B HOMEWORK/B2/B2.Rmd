...
Yes, the respondents' answers are very similar over time. The error-minimizing line would be the 45 degree line although there is some random noise around it.
...
b) We will use the midterm test scores, i.e. `midterm` as our X variable. It is a non-binary variable as it kan take on values between 0 and 100.

c) As stated in the assignment title, we want to predict final exam scores and thus our Y variable will be `final`.

d) The correlation coefficient between the midterm test scores and the finals test scores is positive at .716
